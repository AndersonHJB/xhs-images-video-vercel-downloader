...
- 从最终分享 URL 提取当前 `noteId`。
- 优先读取 `window.__INITIAL_STATE__.note.noteDetailMap[noteId]`。
- 只处理当前笔记对象直属的 `imageList`。
- 不再扫描整页全部 `imageList`，避免混入相关推荐、头像和其他帖子。
- 初始状态解析失败时，只在当前 `noteId` 附近寻找图片列表。
- 最终兜底最多返回当前页面的 `og:image` 封面，不再全局抓图。
- 新增 `npm test`，覆盖“目标帖 7 张 + 推荐帖 15/20 张”的混图场景。
